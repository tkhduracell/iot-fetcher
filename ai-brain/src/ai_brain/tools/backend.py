...
what its devices are doing right now (Home Assistant), what containers are
running on the host (Docker, via docker-proxy), what the user has written
down (Drive), what the rest of the world says (web), and what the agent's own
token budget looks like (usage). All of them read; none of them act.
"""
