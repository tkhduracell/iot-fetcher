  ...
  return (
    <div className="flex-1 min-w-[200px] max-w-[calc(50%-0.25rem)] bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700">
      <div className="flex items-center gap-3">
        {/* Whole info area opens the speaker view for this zone — the room name
            alone was too small a target and read as plain text. */}
        <a
          href="#speakers"
          title={`Visa högtalare: ${displayRoomName}`}
          className="group flex items-center gap-3 flex-1 min-w-0 -m-1 p-1 rounded-md cursor-pointer transition-colors hover:bg-gray-100 dark:hover:bg-gray-700/60 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
        >
          {isHdmiInput ? (
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1">
              <span className="font-medium text-sm text-gray-900 dark:text-gray-100 group-hover:text-indigo-500 dark:group-hover:text-indigo-400 transition-colors truncate">
                {displayRoomName}
              </span>
              {memberCount > 1 && (
                <span className="flex-shrink-0 text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-gray-200 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                  {memberCount}
                </span>
              )}
              <svg
                className="w-3.5 h-3.5 flex-shrink-0 text-gray-400 group-hover:text-indigo-500 dark:group-hover:text-indigo-400 transition-colors"
