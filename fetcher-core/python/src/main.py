...
def main():
    if len(sys.argv) > 1 and sys.argv[1] in ['balboa', 'deco', 'elpris', 'ngenic', 'ngenic_backfill', 'sigenergy', 'aqualink', 'aquatemp', 'airquality', 'tapo', 'sonos', 'backup_influx', 'eufy', 'eufy_snapshot']:
        module_name = sys.argv[1]
        logging.info(f"Running module: {module_name}")
        for m in [balboa, deco, elpris, ngenic, ngenic_backfill, sigenergy, aqualink, aquatemp, airquality, tapo, sonos, backup_influx, eufy, eufy_snapshot]:
