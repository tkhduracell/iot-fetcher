...
sigenergy_host = os.environ.get(
    'SIGENERGY_HOST') or os.environ.get('SIGENERGY_IP') or ''
...
def _sigenergy():
    ...
    if not sigenergy_host:
        logger.error(
            "[sigenergy] SIGENERGY_HOST or SIGENERGY_IP environment variable not set")
        return

    try:
        logger.debug("[sigenergy] Connecting to SigenStore at %s:%d",
                     sigenergy_host, sigenergy_port)
        ...
        try:
            test_result_247 = client.client_247.read_holding_registers(
                30003, 1)
            if test_result_247 is None:
                logger.warning("[sigenergy] No response from unit ID 247")
            else:
                logger.debug(
                    "[sigenergy] Unit ID 247 response: %s", test_result_247)
        ...
        test_result_1 = None
        try:
            test_result_1 = client.client_1.read_holding_registers(
                30500, 1)  # Model type register
            if test_result_1 is None:
                logger.warning("[sigenergy] No response from unit ID 1")
            else:
                logger.debug(
                    "[sigenergy] Unit ID 1 response: %s", test_result_1)
        except Exception as e:
        ...
        if test_result_247 is None and test_result_1 is None:
            logger.error(
                "[sigenergy] Unable to connect to device. Please check:")
            logger.error("  1. Device IP address is correct: %s",
                         sigenergy_host)
            logger.error(
                "  2. Device is powered on and accessible on the network")
            logger.error("  3. Modbus TCP is enabled on the device")
            logger.error(
                "  4. Port %d is not blocked by firewall", sigenergy_port)
            logger.error(
                "  5. Device is configured to accept Modbus connections")
            return
        ...
        try:
            ...
            points.append(Point("sigenergy_grid_power")
                          ...
                          # Keep for debugging/transparency
                          .field("power_to_grid_kw", float(power_to_grid))
                          # Keep for debugging/transparency
                          .field("power_from_grid_kw", float(power_from_grid))
                          )
        ...
        if points:
            write_influx(points)
            logger.info(
                "[sigenergy] Successfully wrote %d data points to InfluxDB", len(points))
        else:
