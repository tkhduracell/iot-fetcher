...
export function housePanels(): cog.Builder<dashboard.Panel>[] {
  ...
  const indoorStat = new StatBuilder()
    ...
    .thresholds(greenRedThresholds(80))
    .overrides([overrideDisplayName('temperature_C', 'Hallen')])
    .withTarget(
  ...
  const outdoorStat = new StatBuilder()
    ...
    .colorScheme(fixedColor('purple'))
    .thresholds(greenRedThresholds(80))
    .overrides([overrideDisplayName('temperature_C', 'Utomhus')])
    .withTarget(
      vmMetric('B', 'ngenic_node_sensor_measurement_value', 'temperature_C', {
  ...
  const bedroomTemp = new TimeseriesBuilder()
    ...
    .withTarget(vmMetric('A', 'bedroom_climate', 'current_temperature_value'))
    .gridPos({ h: 7, w: 8, x: 0, y: 8 });

  // Sovrum (stat)
  const bedroomStat = new StatBuilder()
    .title('')
    .datasource(VM_DS)
    .unit('celsius')
    .colorScheme(fixedColor('purple'))
    .thresholds(greenRedThresholds(80))
    .overrides([overrideDisplayName('current_temperature_value', 'Sovrum')])
    .withTarget(vmMetric('A', 'bedroom_climate', 'current_temperature_value'))
    .timeFrom('30m')
