...
import { greenThreshold, paletteColor, legendBottom, tooltipSingle, SPAN_NULLS_MS } from '../helpers.ts';
...
export function eufyPanels(): cog.Builder<dashboard.Panel>[] {
  ...
  const workingDays = new StatBuilder()
    ...
    .withTransformation({ id: 'labelsToFields', options: { valueLabel: 'device_name' } })
    .withTarget(
      vmMetric('A', 'eufy_device', 'detectionStatisticsWorkingDays', { agg: 'LAST_VALUE' }),
    )
    .timeFrom('30d/d')
  ...
  const events = new StatBuilder()
    ...
    .thresholds(greenThreshold())
    .withTransformation({ id: 'labelsToFields', options: { valueLabel: 'device_name' } })
    .withTarget(vmMetric('A', 'eufy_device', 'detectionStatisticsDetectedEvents'))
    .timeFrom('30d/d')
